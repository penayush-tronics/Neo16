...
Connect power to the VIN and GND pins accordingly; it take can take 3.3V or 5V, but make sure your DIN pin input from the microcontroller/dev board matches this voltage (for example: VIN-5V, DIN-5V). Connect DIN to any one digital pin on the microcontroller/dev board. And that's about it for the wiring; it is really simple to hook up. 
